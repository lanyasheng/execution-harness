...
| 脚本 | Pattern | 用途 |
|------|---------|------|
| `scripts/ralph-init.sh <session-id> [max-iter]` | #1 | 初始化 Ralph 状态（支持 crash 恢复） |
| `scripts/ralph-stop-hook.sh` | #1 | Stop hook，读 stdin 输出 block/continue JSON |
| `scripts/ralph-cancel.sh <session-id> [reason]` | #7 | 发送带 30s TTL 的取消信号 |
| `scripts/tool-error-tracker.sh` | #3 | PostToolUseFailure hook，追踪连续失败 |
| `scripts/tool-error-advisor.sh` | #3 | PreToolUse hook，5 次失败后 block 同一命令 |
| `scripts/doubt-gate.sh` | #13 | Stop hook，检测投机语言强制提供证据 |
| `scripts/post-edit-check.sh` | #15 | PostToolUse hook，编辑后即时跑 linter/type checker |
| `scripts/context-usage.sh <transcript>` | #5 | 从 transcript 尾部 4KB 估算 context 使用率 |
