...
Claude Code 自身的 `conversationRecovery.ts` 已处理消息链修复（孤立 tool_use 补 placeholder），hook 只需聚焦 ralph.json 和 lock 这些外部状态。 → [详见](references/crash-recovery.md)
...
与 crash recovery 的区别：crash recovery 恢复执行状态，stale session daemon 抢救认知状态。 → [详见](references/stale-session.md)
...
→ [详见](references/mcp-reconnection.md)
...
→ [详见](references/graceful-degradation.md)
...
→ [详见](references/model-fallback.md)
...
pane 输出 "Rate limited. Press Enter to retry"。rate-limit-recovery.sh 扫描 pane（capture-pane -S -20），匹配 "rate limit"。二次验证最后 5 行不含 [y/N]、confirm 等确认提示，发送 Enter 恢复。
</example>

<example>
场景：新 session 启动发现上次 crash 残留
检测到 sessions/abc123/ralph.json 状态 active=true，last_checked_at 距今 3 小时（>7200s 阈值）。设 active=false, deactivation_reason="stale"。同时发现 .claims/src_auth.lock 已存在 20 分钟（>15min），自动删除。检测到 .working-state/current-plan.md 存在，通过 additionalContext 注入"上次 session 有中断的计划，请先读取 .working-state/current-plan.md"。
</example>

<example>
场景：MCP 工具断连
agent 调用 mcp__firecrawl__scrape 返回 ECONNREFUSED。MCP 重连机制启动，按 1s → 2s → 4s 间隔重试 3 次，第 3 次成功恢复连接。重试状态记录在 sessions/xxx/mcp-retry-firecrawl.json。如果 5 次全部失败，注入建议："Firecrawl 连接失败，可用 WebFetch 作为替代（不支持 JS 渲染）"。
</example>

<anti-example>
3 个后台 agent 同时遇到 529 过载，全部以相同间隔重试。重试叠加放大服务端压力，529 持续从 2 分钟延长到 15 分钟。后台 session 应直接放弃（exit 0），只记录日志。
</anti-example>
