...
if [ "$PREV_TOOL" = "$TOOL" ] && [ "$PREV_HASH" = "$INPUT_HASH" ] && [ "$PREV_COUNT" -ge "$HARD_THRESHOLD" ]; then
  echo "{\"decision\":\"block\",\"reason\":\"BLOCKED: '$TOOL' with this input has failed $PREV_COUNT times. Last error: $PREV_ERROR. You MUST use a completely different approach.\"}"
else
  echo '{"continue":true}'
fi
