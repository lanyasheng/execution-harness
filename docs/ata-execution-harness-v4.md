...
- 38 个 pattern 从 4 个开源项目蒸馏而来，不是凭空设计。17 个脚本有 90 个 pytest 测试覆盖。
...
这 6 个场景不是极端 case。用 Claude Code 做稍微复杂一点的任务，大概率会撞上其中几个。
...
不是偶发。改完 2 个文件，agent 觉得已经展示了"怎么改"，对它来说任务完成了。但你的 7 个文件是一个原子任务——改了 2 个不改剩下 5 个，不如一个都不改。

Interactive 模式下每次 `end_turn` 都是退出点。没有外力阻止，agent 会在每个"看起来差不多了"的节点停下来。→ Pattern 1.1 Ralph Persistent Loop
...
原因比你想的简单。`PostToolUseFailure` 只给 agent 看到这一次的错误。没有计数器告诉它"你用同样的方式失败了 12 次"。对 agent 来说每次都是全新的"再试试"。→ Pattern 2.1 Tool Error Escalation，3 次提示换策略，5 次直接 block。
...
压缩本身没错——context 是有限的。问题在于你控制不了它留什么丢什么。"用 Redis"这个结论可能留下了。但"为什么排除 Memcached"？丢了。推理既不是代码也不是结论，压缩算法眼里它就是可删的废话。→ Pattern 3.1 Handoff Documents，决策写磁盘，压缩删不掉。
...
经典的 lost update。数据库领域有锁、MVCC、CAS，但 agent 之间共享的是裸文件系统——Read 读、Write 写，中间零并发控制。更隐蔽的是 agent 压根不知道其他 agent 存在。A 不知道 B 在改同一个文件，也不会检查。→ Pattern 4.3 File Claim and Lock。
...
没人按。那个 pane 就挂在那。30 分钟后你看了一眼，还在等。另外两个 agent 可能做完了，但如果它们依赖这个 agent 的输出，整条流水线就停了。→ Pattern 5.1 Rate Limit Recovery，脚本扫描 tmux pane 自动发 Enter。
...
3 个互相依赖的文件，每个都有类型错误，错误级联放大。写的时候发现是 1 分钟的事，3 个文件之后回头修可能要半小时。→ Pattern 6.1 Post-Edit Diagnostics，每次编辑后自动跑 linter/type checker。
...
跨 agent、跨 session、跨 hook 的通信为什么不用数据库？不用消息队列？

因为 hook 是 bash 脚本。bash 读写文件是天生的。引入 Redis 意味着每个脚本都得装 `redis-cli`，部署变复杂，调试变困难。排查 hook 问题时，`cat` 看状态文件、`jq` 解析 JSON，30 秒搞定。状态在 Redis 里？得连上去 `HGETALL`，出了问题分不清是 hook 逻辑错了还是 Redis 连接断了。
...
为什么 rename？`jq ... > file` 写到一半 kill 掉，你就有一个半截 JSON。下次 hook 读到就崩。rename 是原子的——要么整个文件更新，要么什么都没变。

代价是什么？文件 I/O 比内存操作慢。5 个 agent 同时更新状态文件会有竞争。Pattern 4.3 File Claim and Lock 部分缓解，但不完美。真正的高并发场景（几十个 agent）可能需要升级到数据库——不过在 Execution Harness 的典型场景（2-5 个 agent，每秒几次状态更新）里，文件系统够用。
...
<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;font-size:13px;margin:20px 0;background:#fafafa;border:1px solid #e5e7eb;border-radius:12px;padding:24px">
<div style="text-align:center;font-weight:700;font-size:15px;color:#111;margin-bottom:16px">Ralph 的 4 个安全阀</div>
<div style="display:flex;gap:10px;flex-wrap:wrap">
<div style="flex:1;min-width:120px;border-radius:8px;padding:10px;background:#fef2f2;border:1px solid #fca5a5;text-align:center">
...
<div style="flex:1;min-width:120px;border-radius:8px;padding:10px;background:#fefce8;border:1px solid #fde68a;text-align:center">
...
<div style="flex:1;min-width:120px;border-radius:8px;padding:10px;background:#eff6ff;border:1px solid #93c5fd;text-align:center">
...
<div style="flex:1;min-width:120px;border-radius:8px;padding:10px;background:#f0fdf4;border:1px solid #86efac;text-align:center">
...
状态文件不存在？JSON 解析失败？Session ID 拿不到？拿不到 hook 的输入？
...
反直觉吧？安全机制大部分时候选择"不起作用"。但替代方案更差：hook 拿不到状态文件就 block。新环境第一次跑 Claude Code，session 目录还没建，所有 hook 全拦。体验就是"什么都干不了"。然后你卸了整套 hook。

### M6: 干预要和任务复杂度成正比

5 分钟改个 typo 需要什么？打开文件，改一行，commit。不需要 Ralph 持续执行循环，不需要 doubt gate 检测投机语言，不需要 post-edit diagnostics 检查类型。

2 小时的跨 15 个文件的重构呢？全都需要。
...
为什么不全部启用？因为 hook 有开销。改个 README 触发 `tsc` 类型检查，浪费时间。Ralph 在简单任务中更麻烦——agent 在"我改完了"和"hook 说不许停"之间拉扯几轮，最后憋出一些无意义的"额外改进"来满足要求。你以为在提质量，其实在消耗 token。

### M7: 先观测，再干预

先部署 `tool-error-tracker.sh`（PostToolUseFailure hook）记录失败次数，再部署 `tool-error-advisor.sh`（PreToolUse hook）阻止重试。顺序不能反。

为什么？如果你只装了 advisor 没装 tracker，advisor 要读错误计数文件来决定是否 block。但没有 tracker 去写这个文件，advisor 永远读到"文件不存在"，然后按 M5 放行——永远不会触发 block。
...
和内置压缩什么关系？互补。内置压缩管"怎么高效用 context window"，handoff 管"什么东西不许丢"。不冲突。
...
Hook 能切换模型吗？不能。Claude Code 的 hook API 没有这个接口。
...
为什么要标？因为静默失效是最差的结果。你以为"hook 会自动降级到更强的模型"，基于这个假设安排了过夜任务。结果能力不存在，任务失败了 8 小时你才知道。
...
38 个 pattern 分布在 6 个独立的轴上。每个轴是一个独立的 Claude Code skill——你可以只装 execution-loop 解决"agent 总是提前退出"的问题，完全不碰其他 5 个轴。
...
### 38 个 Pattern：每个轴里到底有什么

Pattern 分三种：**script**（bash 脚本，配到 settings.json 就能用）、**design**（设计指南 + 伪代码，按场景适配）、**config**（配置模板，改路径就行）。

<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;font-size:13px;margin:20px 0;background:#fafafa;border:1px solid #e5e7eb;border-radius:12px;padding:24px">
<div style="text-align:center;font-weight:700;font-size:15px;color:#111;margin-bottom:20px">38 Pattern 分类总览</div>
...
<div style="font-size:24px;font-weight:700;color:#3b82f6">20</div>
...
<span style="background:#dbeafe;border-radius:4px;padding:1px 6px;font-size:10px;color:#3b82f6">5 design</span>
</div>
</div>
...
#### Context & Memory — 7 个 pattern，治"压缩后失忆"
...
#### Error Recovery — 6 个 pattern，治"挂死"和"crash 丢进度"
...
50 轮上限够不够？看任务。简单的 10 轮够了，跨 20 个文件的重构要 200 轮。没有万能数字。

### Tool Error Escalation (Pattern 2.1)

容器没装 cargo。Agent 跑 `cargo build`，失败，下一步又是 `cargo build`。参数一模一样，结果一模一样。5 次、10 次。

为什么会这样？`PostToolUseFailure` 只让 agent 看到这一次的错误。没有人告诉它"你用同样的方式失败了 5 次"。对 agent 来说，每次都是一个独立的"再试试"。
...
**Rejected** 段价值最高。"选了 Redis"是结论，10 个字够了。"为什么不选 Memcached"是推理——压缩会丢的就是这部分。不写 Rejected 会怎样？下一个 agent 看到"用 Redis"，问"为什么不用 Memcached"，又花 5 轮讨论同一个问题。
...
第三，跨阶段矛盾。Stage-2 推翻了 stage-1 的决策但没更新 stage-1 的 handoff。后面的 agent 读到两份矛盾的 handoff——信谁？这个问题 Execution Harness 当前没解决。
...
没装 linter 怎么办？按 M5 静默跳过。不会因为少了 `ruff` 就不让你写代码。
...
38 个 pattern 不是从零设计的。6 个源，每个贡献了不同视角。
...
- **编排逻辑**（DAG、fan-in/fan-out）——那是编排层的事。Execution Harness 管执行层，不管任务调度。
...
从 Claude Code 源码里提取的几个关键行为，写 hook 时不知道会踩坑。

### PreToolUse 不只是 Allow/Deny

大多数人以为 PreToolUse hook 只能放行或拦截。实际上它支持三种响应：
...
`updatedInput` 打开了新的可能——hook 不只是门卫（放行/拦截），还可以是改装工（修改工具参数）。比如给 bash 命令自动加 `timeout 60`，给 `rm` 改成 `rm --dry-run` 先看看会删什么。

### 多个 Hook 同时响应时的聚合规则

装了 3 个 PreToolUse hook，它们的输出怎么合并？
...
对 hook 开发者的影响：如果你的 hook 依赖 compact 事件（比如 Pattern 3.7 在 compact 后跑审计），compact 被 circuit breaker 跳过时你的 hook 不会触发。context 会持续膨胀直到 API 返回 413 触发 Reactive Compact。

### 前台重试、后台放弃

529（API overloaded）不是 429（你被限速了）。529 是服务端全局过载——你重试只会火上浇油。

Claude Code 的做法：前台请求（用户直接发起）遇 529 会重试（最多 3 次）。后台请求（摘要生成、安全分类器、background compaction）遇 529 直接放弃。

同样的思路适用于 async hook。`async: true` 的 hook 本质是后台任务——失败了不要在脚本里加 retry loop，直接 exit 0 让 agent 继续。系统最忙的时候，后台重试是最不该做的事。
...
搭自己的 execution harness？这 8 个坑最容易踩。
