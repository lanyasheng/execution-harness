...
**为什么**：Fork child 继承完整的 parent context。如果允许递归 Fork，context 成本指数增长。Claude Code 内部的实现中，Fork tool 在 child agent 的 tool pool 中仍然可见（为了 cache sharing），但在 call time 被 guard 阻止。
