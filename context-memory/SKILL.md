...
<example>
场景: Handoff 保护决策不丢失
20 轮 Redis 方案讨论。第 16 轮定方案：Redis Cluster 6 节点，否决 Codis 和 Sentinel。第 17 轮写 handoff 到 sessions/xxx/handoffs/stage-2.md（五段：Decided/Rejected/Risks/Files/Remaining）。第 20 轮 Full Compact 截断 context。Compact 后 agent 读 stage-2.md，恢复全部决策，不会重提 Codis。
</example>

<example>
场景: Compaction 质量审计发现决策丢失
decisions.jsonl 最近 5 条含关键词 "Redis Cluster"、"Codis"、"Sentinel"。Full Compact 后 grep compact summary，发现 "Codis" 未出现。审计 hook 注入 additionalContext："[COMPACT AUDIT] 以下决策在压缩中丢失: 否决 Codis（社区停更，Proxy 延迟 +2ms）"。Agent 读到注入信息，不会再提议 Codis。
</example>

<example>
场景: Token 预算阻止 context 溢出
第 30 轮，UserPromptSubmit hook 检测 input_tokens = 170K。按 200K window 计算使用率 85%，注入指令："[CONTEXT BUDGET] 85%, 必须使用 subagent 执行后续任务，禁止直接读大文件"。Agent 切换为 subagent 委派模式，避免 context 在第 35 轮溢出触发紧急 Reactive Compact。
</example>
